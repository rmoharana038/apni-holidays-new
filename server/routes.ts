...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  try {
    console.log("📦 Incoming package data:", req.body); // 👀 STEP 1: print input

    const packageData = insertPackageSchema.parse(req.body); // ✅ STEP 2: validate
    const pkg = await storage.createPackage(packageData);    // ✅ STEP 3: insert into DB
    res.json(pkg);
  } catch (error) {
    console.error("❌ Validation failed:", error); // 👀 STEP 4: print error
